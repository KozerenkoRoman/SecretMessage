...
func ApplyBaron(state GameState, action Action, clock Clock, startEventID uint64) (ApplyResult, error) {
	...
	events := []DomainEvent{{
		...
		Payload:   CardPlayedPayload{PlayerID: action.PlayerID, Card: CardBaron, TargetID: action.TargetID},
		Timestamp: clock.Now(),
	}}
	...
	if len(player.Hand) == 0 || len(target.Hand) == 0 {
		return ApplyResult{}, NewError(ErrBaronNoCardsToCompare,
			"player_hand=%d, target_hand=%d", len(player.Hand), len(target.Hand))
	}
	...
	if playerCard > targetCard {
		...
		events = append(events, DomainEvent{
			...
			Payload:   BaronResultPayload{WinnerID: action.PlayerID, LoserID: action.TargetID},
			Timestamp: clock.Now(),
		}, DomainEvent{
			EventID:   startEventID + 3,
			Type:      EventPlayerEliminated,
			Payload:   PlayerEliminatedPayload{PlayerID: action.TargetID, Reason: ReasonBaronLost},
			Timestamp: clock.Now(),
		})
		...
		events = append(events, DomainEvent{
			EventID:   startEventID + 2,
			Type:      EventBaronResult,
			Payload:   BaronResultPayload{WinnerID: action.TargetID, LoserID: action.PlayerID},
			Timestamp: clock.Now(),
		}, DomainEvent{
			EventID:   startEventID + 3,
			Type:      EventPlayerEliminated,
